...
import com.nachigo.pooltupi.Services.BlockFoundService;
import com.nachigo.pooltupi.R;
import com.nachigo.pooltupi.Services.SampleBootReceiver;
...
public class Settings extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Switch SPainel = findViewById(R.id.SPainel);
        ...
        if (settings.getBoolean("OpenPainel", false)) {
            ...
            SPainel.setChecked(true);
        } else {
            SPainel.setChecked(false);
        }
        ...
        SPainel.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if ((SPainel.isChecked())){
                    SharedPreferences settings = getSharedPreferences("tupiniquim", 0);
                    SharedPreferences.Editor editor = settings.edit();
                    editor.putBoolean("OpenPainel", true);
                    editor.commit();
                }
                if (!(SPainel.isChecked())){
                    SharedPreferences settings = getSharedPreferences("tupiniquim", 0);
                    SharedPreferences.Editor editor = settings.edit();
                    editor.putBoolean("OpenPainel", false);
                    editor.commit();
                }
            }
        });
